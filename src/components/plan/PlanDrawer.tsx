...
export function PlanDrawer() {
  ...
  return (
    <>
      <style>{`@keyframes pdIn{from{transform:translateX(100%)}to{transform:translateX(0)}}@keyframes pdFade{from{opacity:0}to{opacity:1}}`}</style>
      ...
      <div style={{ position: 'fixed', top: 0, right: 0, bottom: 0, width: 434, maxWidth: '100vw', zIndex: 91, background: '#fff', boxShadow: '-12px 0 40px rgba(20,18,12,0.16)', display: 'flex', flexDirection: 'column', animation: 'pdIn .28s cubic-bezier(.22,1,.36,1) both' }}>
        ...
        <div style={{ flex: 1, minHeight: 0, overflowY: 'auto', overflowX: 'hidden', padding: '0 24px 24px', display: 'flex', flexDirection: 'column', gap: 22 }}>
          ...
          {/* live math */}
          <div style={{ margin: '0 -24px', borderBottom: '1px solid #f1f0ed' }}><LiveMath c={c} d={d} idSuffix={p.id} /></div>

          {/* diagram */}
          <div style={{ margin: '0 -24px', borderTop: '1px solid #f1f0ed', borderBottom: '1px solid #f1f0ed', padding: '16px 4px 18px' }}><RrDiagram c={c} d={d} compact /></div>
