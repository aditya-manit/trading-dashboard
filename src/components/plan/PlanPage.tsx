...
function Verdict({ e }: { e: CalendarEvent }) {
  ...
  const dir = btc?.dir; // up | down | flat — direction in the "hot" scenario
  const cond = firstWord(ins.condition || 'hot');
  ...
  return (
    <span style={{ display: 'inline-flex', alignItems: 'center', fontWeight: 600, fontSize: 11.5, color: '#8b8578', whiteSpace: 'nowrap' }}>
      ...
      <HoverTip tip={econ}>{econIcon}</HoverTip>{arrow}
      <HoverTip tip={fed}>{fedIcon}</HoverTip>{arrow}
      <b style={{ color: dirCol, fontWeight: 800 }}>{btcTxt}</b>
    </span>
  );
}
...
function StripCard({ e, loading, def, v2 }: { e: CalendarEvent; loading: boolean; def?: string; v2?: boolean }) {
  ...
  return (
    <HoverTip width={252} tip={
      ...
      <span style={{ display: 'inline-flex', alignItems: 'center', gap: 5, cursor: 'help', whiteSpace: 'nowrap' }}>
        ...
        {reg.rate != null && <span style={{ fontWeight: 800, fontSize: 10.5, color: '#1a1813', fontVariantNumeric: 'tabular-nums' }}>{reg.rate.toFixed(2)}%</span>}
      </span>
    </HoverTip>
  );
}
        ...
        {(calLoading || stripEvents.length > 0) && (
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4,1fr)', gap: 10 }}>
            {calLoading
              ...
              : stripEvents.map((e, i) => <StripCard key={i} e={e} loading={insightsLoading} def={defMap?.[e.title]} v2/>)}
          </div>
        )}
      </div>
