...
function LoginInner() {
  ...
  // NEXT_PUBLIC_* are inlined at build time — if a deploy was built without them
  // the button can't work, so say so instead of silently no-opping.
    ...
    <div style={{ minHeight: '100vh', display: 'grid', placeItems: 'center', background: '#e9e8e4', padding: 24 }}>
      <div style={{ width: '100%', maxWidth: 380, background: '#fff', border: '1px solid #f0efec', borderRadius: 22, boxShadow: '0 8px 40px rgba(20,18,12,0.08)', padding: '34px 30px 30px', textAlign: 'center' }}>
        ...
        <button
          ...
          style={{ width: '100%', display: 'inline-flex', alignItems: 'center', justifyContent: 'center', gap: 11, background: '#fff', border: '1px solid #ddd9d0', borderRadius: 12, padding: '12px 16px', fontFamily: 'inherit', fontWeight: 700, fontSize: 14, color: '#1a1813', cursor: (busy || !configured) ? 'default' : 'pointer', opacity: (busy || !configured) ? 0.55 : 1, transition: 'background .15s' }}
        >
