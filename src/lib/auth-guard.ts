...
const MFA_MAX_AGE_S = 14 * 24 * 3600; // must match proxy.ts
...
export async function requireOwner(): Promise<NextResponse | null> {
  // DEV-ONLY bypass — only in local `next dev`; inert on every Vercel build
  // (NODE_ENV=production there). See CLAUDE.md "Dev auth bypass".
  if (process.env.DISABLE_AUTH === 'true' && process.env.NODE_ENV !== 'production') return null;
