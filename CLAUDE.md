...
- **Each card's tier tag REPLACES the old HIGH badge** (everything shown is high-impact, so HIGH was redundant). `relevanceTag` (`calendar-filter.ts`) → `US MACRO` / `CENTRAL BANK`, both in the brand purple `#7c5cff` (user chose a single accent over two-colour), rendered by `Tag` in the badge's old text style. Mirrors the `isBtcRelevant` gate.
