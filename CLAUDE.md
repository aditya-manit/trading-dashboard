...
- **Insight enrichment is two-tier** (`lib/event-insight.ts`, model `claude-haiku-4-5-20251001`, `fetch` to Messages API — no SDK dep), both cached in-process by `currency|title`:
  ...
  - **`enrichPrints`** — the 2 most recent occurrence dates. **Per-event `web_search` server tool** (capped 5 concurrent), verified. Run **only for the strip's 4 cards** (the expensive bit). The route fills each date's `%` from the Gate candle map (`btcDailyMoves`, cached 1h, recomputed per request).
  - Caching: web search only fires on a cold cache for a newly-seen event (warm `/api/calendar` ≈ 20ms, cold ≈ 10–35s). Without `ANTHROPIC_API_KEY` both tiers are skipped → cards show forecast only.
- **Each card has a relevance tag** (`relevanceTag` in `calendar-filter.ts`, rendered by `Tag`): `US MACRO` (amber) for USD, `CENTRAL BANK` (purple) for non-USD CB decisions — mirrors the `isBtcRelevant` gate.
