...
- **Drawer has an Upcoming / Released toggle** (`newsTab`). Released ("Earlier this week") shows already-fired relevant events in `ReleasedCard`: recessed tint (`#f8f6f2`), 4-col spec table — Forecast | value | Actual | value — then If-`<condition>` | reaction | Reaction | **the realized reaction**. The **Actual is the value itself coloured with a leading caret** (▲ red `#df5338` Hot / ▼ green `#1f9d55` Soft; neutral, no caret, for In line) — handoff 18, no chip. The `If <condition>` label is forced to a single word. Data via `useCalendarReleased` → `/api/calendar/released` → `enrichReleased` (Claude **web search** for the real actual figure + how BTC/stocks actually moved; verified — unconfirmed actuals aren't cached; persisted in the same `.cache/event-insight.json` under `released`; bounded to the 4 most-recent). Chip colour = `bearishForBtc` (red) vs bullish (green); arrow ▲ Hot / ▼ Soft.
