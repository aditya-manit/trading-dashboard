...
- **News header = V5 "departure board"** (`NewsHeader`, handoff 17): pulsing green dot + MARKET NEWS/SAMPLE FEED · NEXT RELEASE (event + currency·impact) · big seconds-ticking `CountdownFull` + UNTIL RELEASE · time-until **progress bar** (fraction between the previous relevant event and the next) · THIS WEEK impact legend (counts of upcoming High/Med/Low) · View all. Replaces the old bell banner.
