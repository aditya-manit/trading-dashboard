...
- **Each card's tier tag REPLACES the old HIGH badge** (everything shown is high-impact, so HIGH was redundant). `relevanceTag` (`calendar-filter.ts`) → `US MACRO` (amber) for USD, `CENTRAL BANK` (purple) for non-USD CB decisions, rendered by `Tag` in the badge's old text style. Mirrors the `isBtcRelevant` gate.
