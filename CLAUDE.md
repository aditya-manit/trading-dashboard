...
- **Drawer has an Upcoming / Released toggle** (`newsTab`). Released ("Earlier this week") shows already-fired relevant events in `ReleasedCard`: same white card styling as upcoming, 4-col spec table — Forecast | value | Actual | value — then If-`<condition>` | reaction | Reaction | **the realized reaction**. The **Actual is coloured by the EVENT'S OWN lean** (`surprise`, INDEPENDENT of BTC): Hot/hawkish → red ▲, Soft/dovish → green ▼, In-line → grey `=`. The **Reaction row shows what BTC actually did** (`reaction`) and is kept SEPARATE — the two are independent signals so a divergence (hawkish event, BTC up) is visible; never derive one from the other. A hawkish FOMC reads red ▲ Actual even though BTC may or may not have fallen. Rate decisions return the single policy rate matching the forecast; events with no numeric forecast may show a range. The `If <condition>` label is forced to a single word.
