...
- **Insight enrichment is two-tier** (`lib/event-insight.ts`, model `claude-haiku-4-5-20251001`, `fetch` to Messages API — no SDK dep), both cached in-process by `currency|title`:
  - **`enrichReactions`** — condition + assets ("if hawkish → BTC↓ · USD↑ · SPX↓"). Cheap, **one batched call, NO web search**. Run for **ALL relevant events**, so every card (strip *and* drawer) gets the reaction row. **Deterministic guard `normalizeReaction`** (applied at read time, so it also corrects already-cached wrong values without a cache wipe): the tier-1 reaction is BY DEFINITION the currency-BULLISH / hawkish scenario = risk-off, so it **forces BTC→down, stocks→down, the event's own currency→up**. This stops a model slip from inverting a card — the bug was **Unemployment Rate**, where the model read "low unemployment" as risk-on and showed BTC↑/USD↓/stocks↑; the correct hawkish read is BTC↓/USD↑/stocks↓ (matching the Earnings/NFP siblings). The prompt also now spells out that a STRONG labor reading (low unemployment / high NFP / hot wages) is the bullish, BTC-bearish one.
...
- **⚠️ THE MISSES ARE ALWAYS BEHAVIORS, NOT LAYOUT — a screenshot is NOT proof of done.** Across the handoff-43 editor + handoff-44 heatmap ports, layout was right on the first pass every time; what got missed were the *invisible-in-a-screenshot* behaviors: section-header `:hover`, the date-chip style + its calendar being **clipped** by a group's `overflow:hidden`, preset dropdowns being clipped, **Tab-to-autofill** on the title, and the **two-way data binding** (TP price ⇄ bank target ⇄ thesis Target-rule ⇄ Reward·plan stat). Root cause: verifying visually only, and reading grep *slices* of the dc.html (porting markup but missing the bound handlers). **Port protocol:**
  1. **Read the WHOLE component** — render fn + **every bound handler** (grep the vals/`renderVals` object for each `on*`: `onKeyDown`/`onInput`/`onClick`/`onSync`…) + the `<style>` block (`:hover`, transitions). Checklist every interactive element + CSS class BEFORE writing code.
  2. **Side-by-side render FIRST, not after pushback:** seed the dc.html AND the React build with the SAME state (for the editor: `localStorage` `tdplan_view='editor'` + `tdplan_tp_draft=<json>`; the dc.html reads those on load) and diff them. This is what finally made the editor exact.
  3. **Interaction sweep before "done":** hover every header, **open every dropdown/popover** (confirm it isn't clipped by a group `overflow:hidden` — if it is, **portal it to `document.body`** with fixed positioning from the anchor rect, like `HoverTip` / `PlanDateModal` / the editor's date calendar + preset menus), Tab through every field, and edit ONE field to confirm connected fields update.
  4. Don't say "done / exact match" until that sweep passes. If the user pastes a change-list, treat it as the authoritative spec; if they say "full port", rebuild the whole component (don't under-scope it as incremental).
