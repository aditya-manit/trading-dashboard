...
- **News header = V5 "departure board"** (`NewsHeader`, handoff 17): pulsing green dot + MARKET NEWS / **LIVE FEED** (not "sample" — it's the real feed) · NEXT RELEASE (event + currency·impact) · big seconds-ticking `CountdownFull` + UNTIL RELEASE · time-until **progress bar** · THIS WEEK legend showing **our tiers** (US Macro sky-blue / Central bank purple counts — not generic High/Med/Low, which we don't surface) · View all. Replaces the old bell banner.
- **Drawer has an Upcoming / Released toggle** (`newsTab`). Released ("Earlier this week") shows already-fired relevant events in `ReleasedCard`: recessed tint (`#f8f6f2`), 4-col spec table — Forecast | value | Actual | value + **Hot/Soft surprise chip** — then If-`<condition>` | reaction | Reaction | **the realized reaction**. Data via `useCalendarReleased` → `/api/calendar/released` → `enrichReleased` (Claude **web search** for the real actual figure + how BTC/stocks actually moved; verified — unconfirmed actuals aren't cached; persisted in the same `.cache/event-insight.json` under `released`; bounded to the 4 most-recent). Chip colour = `bearishForBtc` (red) vs bullish (green); arrow ▲ Hot / ▼ Soft.
