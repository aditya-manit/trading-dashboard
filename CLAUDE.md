...
- **Each card's tier tag REPLACES the old HIGH badge** (everything shown is high-impact, so HIGH was redundant). `relevanceTag` (`calendar-filter.ts`) → `US MACRO` in sky-blue `#0ea5e9`, `CENTRAL BANK` in brand purple `#7c5cff` — two equally-vivid accents, both avoiding green/red (= P&L). Rendered by `Tag` in the badge's old text style. Mirrors the `isBtcRelevant` gate.
